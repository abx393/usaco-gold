...
public class cownav {
	...
	public static void main(String[] args) throws IOException {
		...
		LinkedList<Stait> q = new LinkedList<Stait>();
		q.add(new Stait(0, 0, 2, 0, 0, 1, 0));
		
		while (!q.isEmpty()){
			Stait s = q.removeFirst();
			...
			Stait s2 = new Stait(s.xpos1, s.ypos1, orient1, s.xpos2, s.ypos2, orient2, s.dist+1);
			if (!visited(s2)) {
				markSeen(s2);
				q.add(s2);
			}
			...
			s2 = new Stait(s.xpos1, s.ypos1, orient1, s.xpos2, s.ypos2, orient2, s.dist+1);
			if (!visited(s2)) {
				markSeen(s2);
				q.add(s2);
			}
			...
			s2 = new Stait(new1[0], new1[1], new1[2], new2[0], new2[1], new2[2], s.dist+1);
			if (!visited(s2)) {
				markSeen(s2);
				q.add(s2);
			}
